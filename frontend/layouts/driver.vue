    ...
    <aside class="w-64 bg-white border-r border-slate-200 flex flex-col min-h-screen fixed left-0 top-0 z-30">
      ...
      <div class="px-6 py-5 border-b border-slate-200">
        <div class="flex items-center gap-3">
          <div class="w-9 h-9 rounded-xl bg-amber-500 flex items-center justify-center text-white">
      ...
      <div class="px-3 py-4 border-t border-slate-200">
        <div class="flex items-center gap-3 px-3 py-2 rounded-xl">
          <div class="w-8 h-8 rounded-full bg-amber-100 flex items-center justify-center text-amber-600 font-bold text-sm flex-shrink-0">
            {{ authStore.user?.full_name?.[0] ?? 'В' }}
          </div>
