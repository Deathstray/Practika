...
def cancel_order(
    ...
    o = db.query(Order).filter(Order.id == order_id, Order.employee_id == current_user.id).first()
    if not o:
        raise HTTPException(status_code=404, detail="Заявка не найдена")
    if o.status != "new":
        raise HTTPException(status_code=400, detail="Можно отменить только заявку в статусе 'новая'")
...
def reject_order(
    ...
    if o.status not in ("new", "accepted"):
        raise HTTPException(status_code=400, detail="Нельзя отклонить заявку в текущем статусе")
    o.rejection_reason = data.rejection_reason
    _add_history(db, o, "rejected", current_user, f"Отклонено: {data.rejection_reason}")
    db.commit()
    db.refresh(o)
    return _build_order_out(o)
...
def assign_order(
    ...
    if o.status != "accepted":
        raise HTTPException(status_code=400, detail="Назначить можно только принятую заявку")

    driver = db.query(Driver).filter(Driver.id == data.driver_id, Driver.status == "active").first()
    if not driver:
        raise HTTPException(status_code=404, detail="Водитель не найден или неактивен")

    vehicle = db.query(Vehicle).filter(Vehicle.id == data.vehicle_id, Vehicle.status == "active").first()
    if not vehicle:
        raise HTTPException(status_code=404, detail="Автомобиль не найден или неактивен")

    duration = data.expected_duration_minutes or o.expected_duration_minutes

    conflict_driver = _check_driver_available(db, data.driver_id, o.desired_datetime, duration, order_id)
    if conflict_driver:
        raise HTTPException(
            status_code=409,
            detail=f"Водитель занят: заявка #{conflict_driver.id} пересекается по времени"
        )

    conflict_vehicle = _check_vehicle_available(db, data.vehicle_id, o.desired_datetime, duration, order_id)
    if conflict_vehicle:
        raise HTTPException(
            status_code=409,
            detail=f"Автомобиль занят: заявка #{conflict_vehicle.id} пересекается по времени"
        )
